...
const adminController = {
  ...
  updateUserDomains: async (req, res) => {
    try {
      ...
      const db = getDB();
      const result = await db.collection("users").updateOne({ _id: new ObjectId(userId) }, { $set: { allowedDomains: allowedDomains } });
  ...
  generateInvite: async (req, res) => {
    try {
      ...
      const code = "WELCOME-TRASHMAIL-" + crypto.randomBytes(8).toString("hex").toUpperCase();
      ...
      await auditService.logActivity(new ObjectId(req.user.id), "GENERATE_INVITE", { code, role }, req.user.role);
