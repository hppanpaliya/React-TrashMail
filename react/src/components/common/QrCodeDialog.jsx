...
const QrCodeDialog = ({ open, onClose, value, title = "Scan email address" }) => {
  ...
  // Reset the toggle whenever the dialog opens or the target value changes,
  // so a mode chosen for one email never leaks to the next.
  useEffect(() => {
    setWatchMode(false);
  }, [open, value]);

  const isAddress = typeof value === "string" && value.includes("@");
  const showWatchMode = watchMode && isAddress;
  const encoded = showWatchMode ? `${window.location.origin}${watchPath(value)}` : value || "";

  return (
    <Dialog open={open} onClose={onClose} title={showWatchMode ? "Scan to open watch view" : title} maxWidth="max-w-xs">
