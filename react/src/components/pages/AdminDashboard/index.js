...
const AdminDashboard = () => {
  ...
  const handleGenerateInvite = async () => {
    try {
      const res = await axios.post(`${env.REACT_APP_API_URL}/api/admin/invites`, { role: inviteRole }, { headers: { "x-auth-token": token } });
  ...
  const handleClearLogs = async () => {
    try {
      const response = await axios.delete(`${env.REACT_APP_API_URL}/api/admin/logs`, {
        headers: { "x-auth-token": token },
  ...
  const logColumns = [
    ...
    { id: "userId", label: "User ID", minWidth: 100, sortable: true },
    {
      ...
      minWidth: 100,
      sortable: true,
      format: (value) => <Chip label={value} color={value === "admin" ? "secondary" : "default"} size="small" />,
    },
    ...
    },
    {
      ...
      format: (value) =>
        value && value.length > 0 ? (
          value.map((user, i) => <Chip key={i} label={user} size="small" sx={{ mr: 0.5 }} />)
        ) : (
  ...
  return (
    <Container maxWidth="lg" sx={{ mt: { xs: 2, sm: 4 }, mb: { xs: 2, sm: 4 }, px: { xs: 1, sm: 3 } }}>
      <Typography
        ...
        gutterBottom
        component="div"
        sx={{ color: "text.primary", fontSize: { xs: "1.5rem", sm: "2.125rem" }, mb: { xs: 2, sm: 3 } }}
      >
        Admin Dashboard
      </Typography>
        ...
        <TabPanel value={tabValue} index={0}>
          <Box sx={{ display: "flex", flexDirection: { xs: "column", sm: "row" }, gap: 2, mb: 2, alignItems: { xs: "stretch", sm: "flex-start" } }}>
            ...
            <Button
              variant="contained"
              color="error"
              onClick={() => setOpenClearLogs(true)}
              sx={{ height: { xs: 40, sm: 56 }, minWidth: { xs: "100%", sm: "auto" } }}
            >
              Clear Logs
            </Button>
          </Box>
      ...
      <Dialog open={openClearLogs} onClose={() => setOpenClearLogs(false)} fullWidth maxWidth="sm" PaperProps={{ sx: { m: { xs: 2, sm: 3 } } }}>
        ...
        <DialogContent sx={{ px: { xs: 2, sm: 3 } }}>
          <Typography gutterBottom>Select retention period. Logs older than this will be permanently deleted.</Typography>
          <TextField
            select
            fullWidth
